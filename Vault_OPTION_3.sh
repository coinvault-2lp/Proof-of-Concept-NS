...
#######################################
#
# Depositor will broadcast DepositTx
# after receiving the
...
#######################################
#
# Now the Depositor is intiating Recovery
# as he has lost both Private key & Hardware Token
#
#######################################

echo "-------------"
echo "Simulating the recovery process by Depositor"
echo "as he has lost both Private Key & Hardware Token!"
echo "-------------"

echo "-------------"
echo "Depositor intimates Vault"
echo "to initiate recovery using Option 3" # Customise this
echo "-------------"

echo "-------------"
echo "Vault signs the Partially Signed"
echo "Provisional Tx with its Private Key"
echo "-------------"

VaultSignatureProv=$(./bx input-sign -c bx.cfg $(./bx wif-to-ec $VaultPriv) "$DepositTxRedeemScriptAsm" $ProvTx)

echo "-------------"
echo "Vault Private Key generated Signature for Provisional Tx:"
echo $VaultSignatureProv
echo "-------------"
...
#########################################
#
# Depositor will create a Recovery Tx
# to complete recovery
# 
#########################################

echo "-------------"
echo "Simulating Recovery by Deositor Using " # [Customize this in accordance with Option]
echo "***Option 3*** of Provisonal Tx"
echo "(Corresponds to Option 4 in Figures)"
echo "-------------"
...
echo "-------------"
echo "Depositor sends Recovery Tx to Vault"
echo "for signing"
echo "-------------"

echo "[D] --> Unsigned Recovery Tx --> [V]"

echo "-------------"
echo "Vault signs Recovery Tx and sends"
echo "Partially Signed Recovery Tx to Depositor"
echo "-------------"

VaultSignatureRecov=$(./bx input-sign -c bx.cfg $(./bx wif-to-ec $VaultPriv) "$ProvTxRedeemScriptAsm" $RecovTx)

echo "-------------"
echo "Vault Private Key generated Signature for Recovery Tx:"
echo $VaultSignatureRecov
echo "-------------"

VaultTokenSignatureRecov=$(./bx input-sign -c bx.cfg $(./bx wif-to-ec $VaultTokenPriv) "$ProvTxRedeemScriptAsm" $RecovTx)

echo "-------------"
echo "VaultToken generated Signature for Recovery Tx:"
echo $VaultTokenSignatureRecov
echo "-------------"

echo "[V] --> Fully Signed Recovery Tx --> [D]"

# echo "-------------"
# echo "Depositor signs Partially Signed Recovery Tx"
# echo "received from Vault"
# echo "-------------"
...
# [Customize This in accordance with Option]
RecovTxSigned=$(./bx input-set -c bx.cfg "[$VaultTokenSignatureRecov] [$VaultSignatureRecov] 3 [$ProvTxRedeemScript]" $RecovTx)
...
bitcoin-cli generatetoaddress 2000 "$DepositorAdrs" >/dev/null 2>&1
